...
sub main ( $input = "input" ) {
    ...
    my $topLeft = [0,1];
    my $bottomRight = [elems($map) - 1, elems($map->[0]) - 2];

    (my $rounds1, $map) = goFromStartToGoal($map, $topLeft, $bottomRight);
    say $rounds1;

    (my $rounds2, $map) = goFromStartToGoal($map, $bottomRight, $topLeft);
    say $rounds2;

    (my $rounds3, $map) = goFromStartToGoal($map, $topLeft, $bottomRight);
    say $rounds3;

    say sum($rounds1, $rounds2, $rounds3);
}
main(@ARGV);
exit();

sub goFromStartToGoal($map, $start, $goal) {
    my %memo;
    my $yStart = $start->[0];
    my $xStart = $start->[1];
    my $yGoal = $goal->[0];
    ...
    $memo{"$yStart $xStart"} = 0;
    my @Q = ([$yStart, $xStart]);

    my $reached = undef;
    my $round = 0;
    while (!defined($reached)) {
        my $prevRound = $round++;
        $map = nextMinute($map);
